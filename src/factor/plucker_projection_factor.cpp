#include "plucker_projection_factor.h"
#include "plucker_line_parameterization.h"
...
Eigen::Matrix2d PluckerLineErrorOneCameraTerm::sqrtInfo_;
Eigen::Matrix2d PluckerLineErrorTwoCameraTerm::sqrtInfo_;
...
//    sqrt_info.setIdentity();
    ...
    if (jacobians) {
        ...
        if (jacobians[1]) {
            ...
            jaco_Lc_ex.block(0, 3, 3, 3) = skew_symmetric(
                    Rbc.transpose() * (nb + skew_symmetric(db) * tbc));  // Lc_theta
...
*/
bool
lineProjectionFactor_incamera::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const {
    Eigen::Vector3d Pi(parameters[0][0], parameters[0][1], parameters[0][2]);
    Eigen::Quaterniond Qi(parameters[0][6], parameters[0][3], parameters[0][4], parameters[0][5]);
...
*/
        ...
        if (jacobians[1]) {
            ...
            jaco_Lc_pose.block(0, 3, 3, 3) = skew_symmetric(
                    Rwbj.transpose() * (nw + skew_symmetric(dw) * twbj));  // Lc_theta
...
/*
  ...
  parameters[3]:  line_orth
*/
bool lineProjectionFactor_instartframe::Evaluate(double const *const *parameters, double *residuals,
