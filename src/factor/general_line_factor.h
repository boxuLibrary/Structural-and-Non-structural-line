...
public:
  ...
  bool operator()(const T *const line_ptr, const T *const T_w_i_h_ptr,
    ...
    Vector3T origin = x_dir_.cast<T>() / rho;
    // Vector3T line_c = x_dir_.cast<T>() * ceres::cos(theta) + origin;
    ...
    Vector6T plk_w =
        plk_from_pose(plk_c_h, T_c_w_h.rotationMatrix(), T_c_w_h.pos);

    Vector6T plk_c_t =
        plk_from_pose(plk_w, T_w_c_t.rotationMatrix(), T_w_c_t.pos);

    Vector3T n_c_t = plk_c_t.head(3);
    Vector3T v_c_t = plk_c_t.tail(3);
